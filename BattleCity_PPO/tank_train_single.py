...
class ValueNet(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(4, 32, kernel_size=5, stride=1, padding=2), nn.ReLU(),
...
def train(opt):
    ...
    while iter < opt.num_iters:
        ...
        preimage = torch.tensor(preimage).cuda()
        state = torch.cat(tuple(preimage for _ in range(4)))[None, :, :, :]
        ...
        while not done:
            ...
            #获取下一状态
            cntimage = pre_processing(grab_screen(window_global),WIDTH,HEIGHT)
            cntimage = torch.tensor(cntimage).cuda()
            next_state = torch.cat(tuple(cntimage for _ in range(4)))[None, :, :, :]
